...
func _readFormatXLs(storageDisks []StorageAPI, f *format.XLV3, emptyCheck bool) (formats, indexFixFormats []*format.XLV3, healRequired bool) {
	...
	indexFixFormats = make([]*format.XLV3, len(disks))
	...
	for {
		...
		for i := range storageDisks {
			if errs[i] != nil {
				if emptyCheck && !isNetError(errs[i]) {
					return formats, indexFixFormats, true
				}
		...
		log.Printf("Waiting for format.json from %v endpoints\n", len(storageDisks)-successCount)
	...
	return formats, indexFixFormats, false
}
...
// initFormatXL - initializes or loads format.json for minio FS mode.
//
// It performs below steps;
// (empty format.json means either format.json not found or empty json content)
// 1. When empty format.json in first endpoint:
//     - If all endpoints have empty format.json, then create fresh setup; else wait for admin to heal.
//                                 (or)
...
func _initFormatXL(setCount, setSize int) error {
	...
	if thisEndpoint == nil {
		panic("unable to find local endpoint")
	}
	...
	formats, indexFixFormats, healRequired := readFormatXLs(storageDisks, thisFormat, emptyCheck)
